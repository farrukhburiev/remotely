...
import farrukh.remotely.databinding.FragmentProfileBinding
...
class ProfileFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        if (user.get(user.size - 1).name == "") {
            ...
            binding.userName.text = user.get(user.size - 1).name
            binding.password.text = user.get(user.size - 1).password
        }
        return binding.root
    }
